...
def add_lora(model: torch.nn.Module, rank: int, alpha: float, 
    ...
    for layer in model.layer:
        for name in modules_to_adapt.get("attention", []):
            if hasattr(layer.attention, name):
                linear = getattr(layer.attention, name)
                setattr(layer.attention, name, LinearLoRA(linear, rank, alpha))
        for name in modules_to_adapt.get("ffn", []):
            if hasattr(layer.ffn, name):
                linear = getattr(layer.ffn, name)
                setattr(layer.ffn, name, LinearLoRA(linear, rank, alpha))
    return model
